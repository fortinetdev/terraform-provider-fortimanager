...
func resourceObjectRouterAspathListCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectRouterAspathList(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectRouterAspathList(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectRouterAspathListUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectRouterAspathList(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectRouterAspathList resource: %v", err)
	}
...
func resourceObjectRouterAspathListDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectRouterAspathList(mkey, paradict, wsParams)
	if err != nil {
