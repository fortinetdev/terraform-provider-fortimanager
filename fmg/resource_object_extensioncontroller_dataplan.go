...
func resourceObjectExtensionControllerDataplanCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectExtensionControllerDataplan(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectExtensionControllerDataplan(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectExtensionControllerDataplanUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectExtensionControllerDataplan(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectExtensionControllerDataplan resource: %v", err)
	}
...
func resourceObjectExtensionControllerDataplanDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectExtensionControllerDataplan(mkey, paradict, wsParams)
	if err != nil {
