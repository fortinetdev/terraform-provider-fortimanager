...
func resourcePackagesFirewallCentralSnatMapSort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallCentralSnatMapSortCreateUpdate,
		Read:   resourcePackagesFirewallCentralSnatMapSortRead,
		Update: resourcePackagesFirewallCentralSnatMapSortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallCentralSnatMapSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallCentralSnatMapSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallCentralSnatMap: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallCentralSnatMapSortRead(d, m)
}

func resourcePackagesFirewallCentralSnatMapSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallCentralSnatMapSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallCentralSnatMap sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallCentralSnatMap List for (%s): %s", d.Id(), err)
		}
	} else {
