...
func resourceSystemSyslogCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemSyslog(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemSyslog(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemSyslogUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemSyslog(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemSyslog resource: %v", err)
	}
...
func resourceSystemSyslogDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemSyslog(mkey, paradict, wsParams)
	if err != nil {
