...
func resourceObjectFirewallAddrgrp6TaggingCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallAddrgrp6Tagging(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallAddrgrp6Tagging(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallAddrgrp6TaggingUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallAddrgrp6Tagging(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallAddrgrp6Tagging resource: %v", err)
	}
...
func resourceObjectFirewallAddrgrp6TaggingDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallAddrgrp6Tagging(mkey, paradict, wsParams)
	if err != nil {
