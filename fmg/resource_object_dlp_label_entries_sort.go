...
func resourceObjectDlpLabelEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectDlpLabelEntriesSortCreateUpdate,
		Read:   resourceObjectDlpLabelEntriesSortRead,
		Update: resourceObjectDlpLabelEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectDlpLabelEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectDlpLabelEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectDlpLabelEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectDlpLabelEntriesSortRead(d, m)
}

func resourceObjectDlpLabelEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectDlpLabelEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectDlpLabelEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectDlpLabelEntries List for (%s): %s", d.Id(), err)
		}
	} else {
