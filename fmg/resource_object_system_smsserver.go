...
func resourceObjectSystemSmsServerCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSystemSmsServer(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSystemSmsServer(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSystemSmsServerUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSystemSmsServer(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSystemSmsServer resource: %v", err)
	}
...
func resourceObjectSystemSmsServerDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSystemSmsServer(mkey, paradict, wsParams)
	if err != nil {
