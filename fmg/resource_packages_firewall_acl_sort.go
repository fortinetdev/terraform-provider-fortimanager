...
func resourcePackagesFirewallAclSort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallAclSortCreateUpdate,
		Read:   resourcePackagesFirewallAclSortRead,
		Update: resourcePackagesFirewallAclSortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallAclSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallAclSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallAcl: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallAclSortRead(d, m)
}

func resourcePackagesFirewallAclSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallAclSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallAcl sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallAcl List for (%s): %s", d.Id(), err)
		}
	} else {
