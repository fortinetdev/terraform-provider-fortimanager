...
func resourcePackagesCentralDnatSort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesCentralDnatSortCreateUpdate,
		Read:   resourcePackagesCentralDnatSortRead,
		Update: resourcePackagesCentralDnatSortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesCentralDnatSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesCentralDnatSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesCentralDnat: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesCentralDnatSortRead(d, m)
}

func resourcePackagesCentralDnatSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesCentralDnatSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesCentralDnat sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesCentralDnat List for (%s): %s", d.Id(), err)
		}
	} else {
