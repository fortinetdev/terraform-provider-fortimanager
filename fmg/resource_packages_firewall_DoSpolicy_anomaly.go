...
func resourcePackagesFirewallDosPolicyAnomalyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdatePackagesFirewallDosPolicyAnomaly(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreatePackagesFirewallDosPolicyAnomaly(obj, paradict, wsParams)
		if err != nil {
...
func resourcePackagesFirewallDosPolicyAnomalyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdatePackagesFirewallDosPolicyAnomaly(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating PackagesFirewallDosPolicyAnomaly resource: %v", err)
	}
...
func resourcePackagesFirewallDosPolicyAnomalyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeletePackagesFirewallDosPolicyAnomaly(mkey, paradict, wsParams)
	if err != nil {
