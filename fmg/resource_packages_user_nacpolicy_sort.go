...
func resourcePackagesUserNacPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesUserNacPolicySortCreateUpdate,
		Read:   resourcePackagesUserNacPolicySortRead,
		Update: resourcePackagesUserNacPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesUserNacPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesUserNacPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesUserNacPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesUserNacPolicySortRead(d, m)
}

func resourcePackagesUserNacPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesUserNacPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesUserNacPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesUserNacPolicy List for (%s): %s", d.Id(), err)
		}
	} else {
