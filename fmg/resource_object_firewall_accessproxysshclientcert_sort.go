...
func resourceObjectFirewallAccessProxySshClientCertSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectFirewallAccessProxySshClientCertSortCreateUpdate,
		Read:   resourceObjectFirewallAccessProxySshClientCertSortRead,
		Update: resourceObjectFirewallAccessProxySshClientCertSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectFirewallAccessProxySshClientCertSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectFirewallAccessProxySshClientCertSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectFirewallAccessProxySshClientCert: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectFirewallAccessProxySshClientCertSortRead(d, m)
}

func resourceObjectFirewallAccessProxySshClientCertSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectFirewallAccessProxySshClientCertSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectFirewallAccessProxySshClientCert sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectFirewallAccessProxySshClientCert List for (%s): %s", d.Id(), err)
		}
	} else {
