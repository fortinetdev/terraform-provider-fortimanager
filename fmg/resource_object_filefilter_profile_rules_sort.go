...
func resourceObjectFileFilterProfileRulesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectFileFilterProfileRulesSortCreateUpdate,
		Read:   resourceObjectFileFilterProfileRulesSortRead,
		Update: resourceObjectFileFilterProfileRulesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectFileFilterProfileRulesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectFileFilterProfileRulesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectFileFilterProfileRules: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectFileFilterProfileRulesSortRead(d, m)
}

func resourceObjectFileFilterProfileRulesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectFileFilterProfileRulesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectFileFilterProfileRules sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectFileFilterProfileRules List for (%s): %s", d.Id(), err)
		}
	} else {
