...
func resourceSystemAdminLdapCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemAdminLdap(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemAdminLdap(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemAdminLdapUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemAdminLdap(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemAdminLdap resource: %v", err)
	}
...
func resourceSystemAdminLdapDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemAdminLdap(mkey, paradict, wsParams)
	if err != nil {
