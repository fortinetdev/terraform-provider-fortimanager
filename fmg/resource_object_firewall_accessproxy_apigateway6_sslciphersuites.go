...
func resourceObjectFirewallAccessProxyApiGateway6SslCipherSuitesCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallAccessProxyApiGateway6SslCipherSuites(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallAccessProxyApiGateway6SslCipherSuites(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallAccessProxyApiGateway6SslCipherSuitesUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallAccessProxyApiGateway6SslCipherSuites(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallAccessProxyApiGateway6SslCipherSuites resource: %v", err)
	}
...
func resourceObjectFirewallAccessProxyApiGateway6SslCipherSuitesDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallAccessProxyApiGateway6SslCipherSuites(mkey, paradict, wsParams)
	if err != nil {
