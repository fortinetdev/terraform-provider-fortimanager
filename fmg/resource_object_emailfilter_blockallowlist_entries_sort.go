...
func resourceObjectEmailfilterBlockAllowListEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectEmailfilterBlockAllowListEntriesSortCreateUpdate,
		Read:   resourceObjectEmailfilterBlockAllowListEntriesSortRead,
		Update: resourceObjectEmailfilterBlockAllowListEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectEmailfilterBlockAllowListEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectEmailfilterBlockAllowListEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectEmailfilterBlockAllowListEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectEmailfilterBlockAllowListEntriesSortRead(d, m)
}

func resourceObjectEmailfilterBlockAllowListEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectEmailfilterBlockAllowListEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectEmailfilterBlockAllowListEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectEmailfilterBlockAllowListEntries List for (%s): %s", d.Id(), err)
		}
	} else {
