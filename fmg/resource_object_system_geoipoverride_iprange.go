...
func resourceObjectSystemGeoipOverrideIpRangeCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSystemGeoipOverrideIpRange(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSystemGeoipOverrideIpRange(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSystemGeoipOverrideIpRangeUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSystemGeoipOverrideIpRange(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSystemGeoipOverrideIpRange resource: %v", err)
	}
...
func resourceObjectSystemGeoipOverrideIpRangeDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSystemGeoipOverrideIpRange(mkey, paradict, wsParams)
	if err != nil {
