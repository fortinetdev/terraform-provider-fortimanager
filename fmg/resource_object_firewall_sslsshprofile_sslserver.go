...
func resourceObjectFirewallSslSshProfileSslServerCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallSslSshProfileSslServer(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallSslSshProfileSslServer(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallSslSshProfileSslServerUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallSslSshProfileSslServer(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallSslSshProfileSslServer resource: %v", err)
	}
...
func resourceObjectFirewallSslSshProfileSslServerDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallSslSshProfileSslServer(mkey, paradict, wsParams)
	if err != nil {
