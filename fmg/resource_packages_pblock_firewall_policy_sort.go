...
func resourcePackagesPblockFirewallPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesPblockFirewallPolicySortCreateUpdate,
		Read:   resourcePackagesPblockFirewallPolicySortRead,
		Update: resourcePackagesPblockFirewallPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesPblockFirewallPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesPblockFirewallPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesPblockFirewallPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesPblockFirewallPolicySortRead(d, m)
}

func resourcePackagesPblockFirewallPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesPblockFirewallPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesPblockFirewallPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesPblockFirewallPolicy List for (%s): %s", d.Id(), err)
		}
	} else {
