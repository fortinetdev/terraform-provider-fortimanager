...
func resourceWantempSystemSdwanServiceSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceWantempSystemSdwanServiceSortCreateUpdate,
		Read:   resourceWantempSystemSdwanServiceSortRead,
		Update: resourceWantempSystemSdwanServiceSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceWantempSystemSdwanServiceSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateWantempSystemSdwanServiceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting WantempSystemSdwanService: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceWantempSystemSdwanServiceSortRead(d, m)
}

func resourceWantempSystemSdwanServiceSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadWantempSystemSdwanServiceSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading WantempSystemSdwanService sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading WantempSystemSdwanService List for (%s): %s", d.Id(), err)
		}
	} else {
