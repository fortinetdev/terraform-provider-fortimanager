...
func resourceObjectWebProxyRedirectProfileEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectWebProxyRedirectProfileEntriesSortCreateUpdate,
		Read:   resourceObjectWebProxyRedirectProfileEntriesSortRead,
		Update: resourceObjectWebProxyRedirectProfileEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectWebProxyRedirectProfileEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectWebProxyRedirectProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectWebProxyRedirectProfileEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectWebProxyRedirectProfileEntriesSortRead(d, m)
}

func resourceObjectWebProxyRedirectProfileEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectWebProxyRedirectProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectWebProxyRedirectProfileEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectWebProxyRedirectProfileEntries List for (%s): %s", d.Id(), err)
		}
	} else {
