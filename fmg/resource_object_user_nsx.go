...
func resourceObjectUserNsxCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectUserNsx(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectUserNsx(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectUserNsxUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectUserNsx(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectUserNsx resource: %v", err)
	}
...
func resourceObjectUserNsxDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectUserNsx(mkey, paradict, wsParams)
	if err != nil {
