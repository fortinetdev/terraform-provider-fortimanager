...
func resourceObjectFirewallShapingProfileShapingEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectFirewallShapingProfileShapingEntriesSortCreateUpdate,
		Read:   resourceObjectFirewallShapingProfileShapingEntriesSortRead,
		Update: resourceObjectFirewallShapingProfileShapingEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectFirewallShapingProfileShapingEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectFirewallShapingProfileShapingEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectFirewallShapingProfileShapingEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectFirewallShapingProfileShapingEntriesSortRead(d, m)
}

func resourceObjectFirewallShapingProfileShapingEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectFirewallShapingProfileShapingEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectFirewallShapingProfileShapingEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectFirewallShapingProfileShapingEntries List for (%s): %s", d.Id(), err)
		}
	} else {
