...
func resourcePackagesPblockFirewallLocalInPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesPblockFirewallLocalInPolicySortCreateUpdate,
		Read:   resourcePackagesPblockFirewallLocalInPolicySortRead,
		Update: resourcePackagesPblockFirewallLocalInPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesPblockFirewallLocalInPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesPblockFirewallLocalInPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesPblockFirewallLocalInPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesPblockFirewallLocalInPolicySortRead(d, m)
}

func resourcePackagesPblockFirewallLocalInPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesPblockFirewallLocalInPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesPblockFirewallLocalInPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesPblockFirewallLocalInPolicy List for (%s): %s", d.Id(), err)
		}
	} else {
