...
func resourceObjectWirelessControllerVapVlanNameSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectWirelessControllerVapVlanNameSortCreateUpdate,
		Read:   resourceObjectWirelessControllerVapVlanNameSortRead,
		Update: resourceObjectWirelessControllerVapVlanNameSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectWirelessControllerVapVlanNameSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectWirelessControllerVapVlanNameSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectWirelessControllerVapVlanName: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectWirelessControllerVapVlanNameSortRead(d, m)
}

func resourceObjectWirelessControllerVapVlanNameSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectWirelessControllerVapVlanNameSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectWirelessControllerVapVlanName sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectWirelessControllerVapVlanName List for (%s): %s", d.Id(), err)
		}
	} else {
