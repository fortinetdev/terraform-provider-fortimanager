...
func resourceObjectDlpDictionaryCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectDlpDictionary(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectDlpDictionary(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectDlpDictionaryUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectDlpDictionary(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectDlpDictionary resource: %v", err)
	}
...
func resourceObjectDlpDictionaryDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectDlpDictionary(mkey, paradict, wsParams)
	if err != nil {
