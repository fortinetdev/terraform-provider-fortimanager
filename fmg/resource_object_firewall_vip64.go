...
func resourceObjectFirewallVip64Create(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallVip64(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallVip64(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallVip64Update(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallVip64(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallVip64 resource: %v", err)
	}
...
func resourceObjectFirewallVip64Delete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallVip64(mkey, paradict, wsParams)
	if err != nil {
...
func expandObjectFirewallVip64DynamicMapping(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		if _, ok := d.GetOk(pre_append); ok || d.HasChange(pre_append) {
			t, err := expandObjectFirewallVip64DynamicMappingScope(d, i["_scope"], pre_append)
			if err != nil {
				return result, err
			} else if t != nil {
				tmp["_scope"] = t
