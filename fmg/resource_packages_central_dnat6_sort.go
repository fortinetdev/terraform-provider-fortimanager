...
func resourcePackagesCentralDnat6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesCentralDnat6SortCreateUpdate,
		Read:   resourcePackagesCentralDnat6SortRead,
		Update: resourcePackagesCentralDnat6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesCentralDnat6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesCentralDnat6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesCentralDnat6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesCentralDnat6SortRead(d, m)
}

func resourcePackagesCentralDnat6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesCentralDnat6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesCentralDnat6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesCentralDnat6 List for (%s): %s", d.Id(), err)
		}
	} else {
