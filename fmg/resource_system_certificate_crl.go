...
func resourceSystemCertificateCrlCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemCertificateCrl(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemCertificateCrl(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemCertificateCrlUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemCertificateCrl(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemCertificateCrl resource: %v", err)
	}
...
func resourceSystemCertificateCrlDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemCertificateCrl(mkey, paradict, wsParams)
	if err != nil {
