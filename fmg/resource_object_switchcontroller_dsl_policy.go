...
func resourceObjectSwitchControllerDslPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSwitchControllerDslPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSwitchControllerDslPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSwitchControllerDslPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSwitchControllerDslPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSwitchControllerDslPolicy resource: %v", err)
	}
...
func resourceObjectSwitchControllerDslPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSwitchControllerDslPolicy(mkey, paradict, wsParams)
	if err != nil {
