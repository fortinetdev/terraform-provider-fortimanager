...
func resourceObjectWafSignatureCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectWafSignature(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectWafSignature(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectWafSignatureUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectWafSignature(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectWafSignature resource: %v", err)
	}
...
func resourceObjectWafSignatureDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectWafSignature(mkey, paradict, wsParams)
	if err != nil {
