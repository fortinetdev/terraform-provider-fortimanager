...
func resourceSystemMetadataAdminsCreate(d *schema.ResourceData, m interface{}) error {
	...
	mkey_tf, mkey_ok := d.GetOk("fieldname")
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemMetadataAdmins(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemMetadataAdmins(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemMetadataAdminsUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemMetadataAdmins(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemMetadataAdmins resource: %v", err)
	}
...
func resourceSystemMetadataAdminsDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemMetadataAdmins(mkey, paradict, wsParams)
	if err != nil {
