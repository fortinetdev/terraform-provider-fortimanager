...
func resourcePackagesFirewallDosPolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallDosPolicy6SortCreateUpdate,
		Read:   resourcePackagesFirewallDosPolicy6SortRead,
		Update: resourcePackagesFirewallDosPolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallDosPolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallDosPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallDosPolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallDosPolicy6SortRead(d, m)
}

func resourcePackagesFirewallDosPolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallDosPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallDosPolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallDosPolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {
