...
func resourceObjectFirewallAccessProxyRealserversCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallAccessProxyRealservers(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallAccessProxyRealservers(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallAccessProxyRealserversUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallAccessProxyRealservers(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallAccessProxyRealservers resource: %v", err)
	}
...
func resourceObjectFirewallAccessProxyRealserversDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallAccessProxyRealservers(mkey, paradict, wsParams)
	if err != nil {
