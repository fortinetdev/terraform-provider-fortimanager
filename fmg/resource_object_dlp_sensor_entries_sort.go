...
func resourceObjectDlpSensorEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectDlpSensorEntriesSortCreateUpdate,
		Read:   resourceObjectDlpSensorEntriesSortRead,
		Update: resourceObjectDlpSensorEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectDlpSensorEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectDlpSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectDlpSensorEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectDlpSensorEntriesSortRead(d, m)
}

func resourceObjectDlpSensorEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectDlpSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectDlpSensorEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectDlpSensorEntries List for (%s): %s", d.Id(), err)
		}
	} else {
