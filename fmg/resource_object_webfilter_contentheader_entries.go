...
func resourceObjectWebfilterContentHeaderEntriesCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectWebfilterContentHeaderEntries(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectWebfilterContentHeaderEntries(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectWebfilterContentHeaderEntriesUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectWebfilterContentHeaderEntries(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectWebfilterContentHeaderEntries resource: %v", err)
	}
...
func resourceObjectWebfilterContentHeaderEntriesDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectWebfilterContentHeaderEntries(mkey, paradict, wsParams)
	if err != nil {
