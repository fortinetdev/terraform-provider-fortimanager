...
func resourceSystemLocalInPolicy6Create(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemLocalInPolicy6(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemLocalInPolicy6(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemLocalInPolicy6Update(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemLocalInPolicy6(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemLocalInPolicy6 resource: %v", err)
	}
...
func resourceSystemLocalInPolicy6Delete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemLocalInPolicy6(mkey, paradict, wsParams)
	if err != nil {
