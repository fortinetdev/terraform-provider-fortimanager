...
func resourceObjectVideofilterProfileFiltersSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectVideofilterProfileFiltersSortCreateUpdate,
		Read:   resourceObjectVideofilterProfileFiltersSortRead,
		Update: resourceObjectVideofilterProfileFiltersSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectVideofilterProfileFiltersSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectVideofilterProfileFiltersSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectVideofilterProfileFilters: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectVideofilterProfileFiltersSortRead(d, m)
}

func resourceObjectVideofilterProfileFiltersSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectVideofilterProfileFiltersSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectVideofilterProfileFilters sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectVideofilterProfileFilters List for (%s): %s", d.Id(), err)
		}
	} else {
