...
func resourceObjectCasbSaasApplicationSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectCasbSaasApplicationSortCreateUpdate,
		Read:   resourceObjectCasbSaasApplicationSortRead,
		Update: resourceObjectCasbSaasApplicationSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectCasbSaasApplicationSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectCasbSaasApplicationSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectCasbSaasApplication: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectCasbSaasApplicationSortRead(d, m)
}

func resourceObjectCasbSaasApplicationSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectCasbSaasApplicationSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectCasbSaasApplication sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectCasbSaasApplication List for (%s): %s", d.Id(), err)
		}
	} else {
