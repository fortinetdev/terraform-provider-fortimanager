...
func resourceObjectFmgFabricAuthorizationTemplateCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFmgFabricAuthorizationTemplate(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFmgFabricAuthorizationTemplate(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFmgFabricAuthorizationTemplateUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFmgFabricAuthorizationTemplate(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFmgFabricAuthorizationTemplate resource: %v", err)
	}
...
func resourceObjectFmgFabricAuthorizationTemplateDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFmgFabricAuthorizationTemplate(mkey, paradict, wsParams)
	if err != nil {
