...
func resourceObjectRouterAccessListRuleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectRouterAccessListRule(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectRouterAccessListRule(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectRouterAccessListRuleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectRouterAccessListRule(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectRouterAccessListRule resource: %v", err)
	}
...
func resourceObjectRouterAccessListRuleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectRouterAccessListRule(mkey, paradict, wsParams)
	if err != nil {
