...
func resourceSystemLogRatelimitDeviceCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemLogRatelimitDevice(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemLogRatelimitDevice(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemLogRatelimitDeviceUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemLogRatelimitDevice(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemLogRatelimitDevice resource: %v", err)
	}
...
func resourceSystemLogRatelimitDeviceDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemLogRatelimitDevice(mkey, paradict, wsParams)
	if err != nil {
