...
func resourceObjectWebfilterContentHeaderEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectWebfilterContentHeaderEntriesSortCreateUpdate,
		Read:   resourceObjectWebfilterContentHeaderEntriesSortRead,
		Update: resourceObjectWebfilterContentHeaderEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectWebfilterContentHeaderEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectWebfilterContentHeaderEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectWebfilterContentHeaderEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectWebfilterContentHeaderEntriesSortRead(d, m)
}

func resourceObjectWebfilterContentHeaderEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectWebfilterContentHeaderEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectWebfilterContentHeaderEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectWebfilterContentHeaderEntries List for (%s): %s", d.Id(), err)
		}
	} else {
