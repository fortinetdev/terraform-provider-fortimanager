...
func resourcePackagesFirewallLocalInPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallLocalInPolicySortCreateUpdate,
		Read:   resourcePackagesFirewallLocalInPolicySortRead,
		Update: resourcePackagesFirewallLocalInPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallLocalInPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallLocalInPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallLocalInPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallLocalInPolicySortRead(d, m)
}

func resourcePackagesFirewallLocalInPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallLocalInPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallLocalInPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallLocalInPolicy List for (%s): %s", d.Id(), err)
		}
	} else {
