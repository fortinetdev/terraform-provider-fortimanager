...
func resourcePackagesFirewallLocalInPolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallLocalInPolicy6SortCreateUpdate,
		Read:   resourcePackagesFirewallLocalInPolicy6SortRead,
		Update: resourcePackagesFirewallLocalInPolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallLocalInPolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallLocalInPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallLocalInPolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallLocalInPolicy6SortRead(d, m)
}

func resourcePackagesFirewallLocalInPolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallLocalInPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallLocalInPolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallLocalInPolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {
