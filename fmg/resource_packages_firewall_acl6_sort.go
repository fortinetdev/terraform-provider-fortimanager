...
func resourcePackagesFirewallAcl6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallAcl6SortCreateUpdate,
		Read:   resourcePackagesFirewallAcl6SortRead,
		Update: resourcePackagesFirewallAcl6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallAcl6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallAcl6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallAcl6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallAcl6SortRead(d, m)
}

func resourcePackagesFirewallAcl6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallAcl6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallAcl6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallAcl6 List for (%s): %s", d.Id(), err)
		}
	} else {
