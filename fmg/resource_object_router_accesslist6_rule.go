...
func resourceObjectRouterAccessList6RuleCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectRouterAccessList6Rule(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectRouterAccessList6Rule(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectRouterAccessList6RuleUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectRouterAccessList6Rule(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectRouterAccessList6Rule resource: %v", err)
	}
...
func resourceObjectRouterAccessList6RuleDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectRouterAccessList6Rule(mkey, paradict, wsParams)
	if err != nil {
