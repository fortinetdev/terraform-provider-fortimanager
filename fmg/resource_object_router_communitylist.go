...
func resourceObjectRouterCommunityListCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectRouterCommunityList(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectRouterCommunityList(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectRouterCommunityListUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectRouterCommunityList(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectRouterCommunityList resource: %v", err)
	}
...
func resourceObjectRouterCommunityListDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectRouterCommunityList(mkey, paradict, wsParams)
	if err != nil {
