...
func resourceSystemWorkflowApprovalMatrixCreate(d *schema.ResourceData, m interface{}) error {
	...
	mkey_tf, mkey_ok := d.GetOk("adom_name")
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateSystemWorkflowApprovalMatrix(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateSystemWorkflowApprovalMatrix(obj, paradict, wsParams)
		if err != nil {
...
func resourceSystemWorkflowApprovalMatrixUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateSystemWorkflowApprovalMatrix(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating SystemWorkflowApprovalMatrix resource: %v", err)
	}
...
func resourceSystemWorkflowApprovalMatrixDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteSystemWorkflowApprovalMatrix(mkey, paradict, wsParams)
	if err != nil {
