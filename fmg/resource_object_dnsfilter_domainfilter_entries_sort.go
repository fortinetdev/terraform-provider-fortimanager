...
func resourceObjectDnsfilterDomainFilterEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectDnsfilterDomainFilterEntriesSortCreateUpdate,
		Read:   resourceObjectDnsfilterDomainFilterEntriesSortRead,
		Update: resourceObjectDnsfilterDomainFilterEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectDnsfilterDomainFilterEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectDnsfilterDomainFilterEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectDnsfilterDomainFilterEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectDnsfilterDomainFilterEntriesSortRead(d, m)
}

func resourceObjectDnsfilterDomainFilterEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectDnsfilterDomainFilterEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectDnsfilterDomainFilterEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectDnsfilterDomainFilterEntries List for (%s): %s", d.Id(), err)
		}
	} else {
