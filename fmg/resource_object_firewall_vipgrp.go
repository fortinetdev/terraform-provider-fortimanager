...
func resourceObjectFirewallVipgrpCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		// check existing
		o, err = c.ReadObjectFirewallVipgrp(mkey, paradict)
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallVipgrp(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallVipgrp(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallVipgrpUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallVipgrp(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallVipgrp resource: %v", err)
	}
...
func resourceObjectFirewallVipgrpDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallVipgrp(mkey, paradict, wsParams)
	if err != nil {
...
func expandObjectFirewallVipgrpDynamicMapping(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		if _, ok := d.GetOk(pre_append); ok || d.HasChange(pre_append) {
			t, err := expandObjectFirewallVipgrpDynamicMappingScope(d, i["_scope"], pre_append)
			if err != nil {
				return result, err
			} else if t != nil {
				tmp["_scope"] = t
