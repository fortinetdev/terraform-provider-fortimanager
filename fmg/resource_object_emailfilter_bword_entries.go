...
func resourceObjectEmailfilterBwordEntriesCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectEmailfilterBwordEntries(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectEmailfilterBwordEntries(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectEmailfilterBwordEntriesUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectEmailfilterBwordEntries(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectEmailfilterBwordEntries resource: %v", err)
	}
...
func resourceObjectEmailfilterBwordEntriesDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectEmailfilterBwordEntries(mkey, paradict, wsParams)
	if err != nil {
