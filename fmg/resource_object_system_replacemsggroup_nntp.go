...
func resourceObjectSystemReplacemsgGroupNntpCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSystemReplacemsgGroupNntp(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSystemReplacemsgGroupNntp(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSystemReplacemsgGroupNntpUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSystemReplacemsgGroupNntp(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSystemReplacemsgGroupNntp resource: %v", err)
	}
...
func resourceObjectSystemReplacemsgGroupNntpDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSystemReplacemsgGroupNntp(mkey, paradict, wsParams)
	if err != nil {
