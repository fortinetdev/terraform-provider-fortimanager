...
func resourceObjectCasbUserActivitySort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectCasbUserActivitySortCreateUpdate,
		Read:   resourceObjectCasbUserActivitySortRead,
		Update: resourceObjectCasbUserActivitySortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectCasbUserActivitySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectCasbUserActivitySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectCasbUserActivity: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectCasbUserActivitySortRead(d, m)
}

func resourceObjectCasbUserActivitySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectCasbUserActivitySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectCasbUserActivity sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectCasbUserActivity List for (%s): %s", d.Id(), err)
		}
	} else {
