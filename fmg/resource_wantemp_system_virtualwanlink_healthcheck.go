...
func resourceWantempSystemVirtualWanLinkHealthCheckCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateWantempSystemVirtualWanLinkHealthCheck(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateWantempSystemVirtualWanLinkHealthCheck(obj, paradict, wsParams)
		if err != nil {
...
func resourceWantempSystemVirtualWanLinkHealthCheckUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateWantempSystemVirtualWanLinkHealthCheck(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating WantempSystemVirtualWanLinkHealthCheck resource: %v", err)
	}
...
func resourceWantempSystemVirtualWanLinkHealthCheckDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteWantempSystemVirtualWanLinkHealthCheck(mkey, paradict, wsParams)
	if err != nil {
