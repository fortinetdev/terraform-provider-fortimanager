...
func resourceObjectApplicationListEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectApplicationListEntriesSortCreateUpdate,
		Read:   resourceObjectApplicationListEntriesSortRead,
		Update: resourceObjectApplicationListEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectApplicationListEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectApplicationListEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectApplicationListEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectApplicationListEntriesSortRead(d, m)
}

func resourceObjectApplicationListEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectApplicationListEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectApplicationListEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectApplicationListEntries List for (%s): %s", d.Id(), err)
		}
	} else {
