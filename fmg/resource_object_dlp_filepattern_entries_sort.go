...
func resourceObjectDlpFilepatternEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectDlpFilepatternEntriesSortCreateUpdate,
		Read:   resourceObjectDlpFilepatternEntriesSortRead,
		Update: resourceObjectDlpFilepatternEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectDlpFilepatternEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectDlpFilepatternEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectDlpFilepatternEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectDlpFilepatternEntriesSortRead(d, m)
}

func resourceObjectDlpFilepatternEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectDlpFilepatternEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectDlpFilepatternEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectDlpFilepatternEntries List for (%s): %s", d.Id(), err)
		}
	} else {
