...
func resourcePackagesPblockFirewallLocalInPolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesPblockFirewallLocalInPolicy6SortCreateUpdate,
		Read:   resourcePackagesPblockFirewallLocalInPolicy6SortRead,
		Update: resourcePackagesPblockFirewallLocalInPolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesPblockFirewallLocalInPolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesPblockFirewallLocalInPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesPblockFirewallLocalInPolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesPblockFirewallLocalInPolicy6SortRead(d, m)
}

func resourcePackagesPblockFirewallLocalInPolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesPblockFirewallLocalInPolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesPblockFirewallLocalInPolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesPblockFirewallLocalInPolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {
