...
func resourceObjectDlpFpSensitivityCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectDlpFpSensitivity(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectDlpFpSensitivity(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectDlpFpSensitivityUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectDlpFpSensitivity(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectDlpFpSensitivity resource: %v", err)
	}
...
func resourceObjectDlpFpSensitivityDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectDlpFpSensitivity(mkey, paradict, wsParams)
	if err != nil {
