...
func resourceObjectFirewallShapingProfileShapingEntriesCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallShapingProfileShapingEntries(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallShapingProfileShapingEntries(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallShapingProfileShapingEntriesUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallShapingProfileShapingEntries(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallShapingProfileShapingEntries resource: %v", err)
	}
...
func resourceObjectFirewallShapingProfileShapingEntriesDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallShapingProfileShapingEntries(mkey, paradict, wsParams)
	if err != nil {
