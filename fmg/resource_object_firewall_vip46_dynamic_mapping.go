...
func resourceObjectFirewallVip46DynamicMappingCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallVip46DynamicMapping(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallVip46DynamicMapping(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallVip46DynamicMappingUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallVip46DynamicMapping(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallVip46DynamicMapping resource: %v", err)
	}
...
func resourceObjectFirewallVip46DynamicMappingDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallVip46DynamicMapping(mkey, paradict, wsParams)
	if err != nil {
