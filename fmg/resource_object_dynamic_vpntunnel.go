...
func resourceObjectDynamicVpntunnelCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		// check existing
		o, err = c.ReadObjectDynamicVpntunnel(mkey, paradict)
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectDynamicVpntunnel(obj, mkey, paradict, wsParams)
			if err != nil {
...
func expandObjectDynamicVpntunnelDynamicMapping(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		if _, ok := d.GetOk(pre_append); ok || d.HasChange(pre_append) {
			t, err := expandObjectDynamicVpntunnelDynamicMappingScope(d, i["_scope"], pre_append)
			if err != nil {
				return result, err
			} else if t != nil {
				tmp["_scope"] = t
