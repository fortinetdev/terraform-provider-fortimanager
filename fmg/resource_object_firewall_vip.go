...
func resourceObjectFirewallVipCreate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.CreateObjectFirewallVip(obj, adomv, nil)
...
func resourceObjectFirewallVipUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallVip(obj, adomv, mkey, nil)
	if err != nil {
...
func resourceObjectFirewallVipDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallVip(adomv, mkey, nil)
	if err != nil {
...
func resourceObjectFirewallVipRead(d *schema.ResourceData, m interface{}) error {
	...
	o, err := c.ReadObjectFirewallVip(adomv, mkey, nil)
	if err != nil {
...
func expandObjectFirewallVipDynamicMapping(d *schema.ResourceData, v interface{}, pre string) (interface{}, error) {
	...
	for _, r := range l {
		...
		if _, ok := d.GetOk(pre_append); ok {
			tmp["_scope"], _ = expandObjectFirewallVipDynamicMappingScope(d, i["_scope"], pre_append)
		} else {
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "realservers"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["realservers"], _ = expandObjectFirewallVipDynamicMappingRealservers(d, i["realservers"], pre_append)
		} else {
		...
		if _, ok := d.GetOk(pre_append); ok {
			...
			tmp["srcintf-filter"] = make([]string, 0)
		}
		...
		pre_append = pre + "." + strconv.Itoa(con) + "." + "ssl_cipher_suites"
		if _, ok := d.GetOk(pre_append); ok {
			tmp["ssl-cipher-suites"], _ = expandObjectFirewallVipDynamicMappingSslCipherSuites(d, i["ssl_cipher_suites"], pre_append)
		} else {
