...
func resourceObjectSystemSdnConnectorRouteCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSystemSdnConnectorRoute(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSystemSdnConnectorRoute(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSystemSdnConnectorRouteUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSystemSdnConnectorRoute(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSystemSdnConnectorRoute resource: %v", err)
	}
...
func resourceObjectSystemSdnConnectorRouteDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSystemSdnConnectorRoute(mkey, paradict, wsParams)
	if err != nil {
