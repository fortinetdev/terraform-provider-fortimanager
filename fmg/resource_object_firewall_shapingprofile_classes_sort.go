...
func resourceObjectFirewallShapingProfileClassesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectFirewallShapingProfileClassesSortCreateUpdate,
		Read:   resourceObjectFirewallShapingProfileClassesSortRead,
		Update: resourceObjectFirewallShapingProfileClassesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectFirewallShapingProfileClassesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectFirewallShapingProfileClassesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectFirewallShapingProfileClasses: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectFirewallShapingProfileClassesSortRead(d, m)
}

func resourceObjectFirewallShapingProfileClassesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectFirewallShapingProfileClassesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectFirewallShapingProfileClasses sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectFirewallShapingProfileClasses List for (%s): %s", d.Id(), err)
		}
	} else {
