...
func resourceObjectDlpDictionaryEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectDlpDictionaryEntriesSortCreateUpdate,
		Read:   resourceObjectDlpDictionaryEntriesSortRead,
		Update: resourceObjectDlpDictionaryEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectDlpDictionaryEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectDlpDictionaryEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectDlpDictionaryEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectDlpDictionaryEntriesSortRead(d, m)
}

func resourceObjectDlpDictionaryEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectDlpDictionaryEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectDlpDictionaryEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectDlpDictionaryEntries List for (%s): %s", d.Id(), err)
		}
	} else {
