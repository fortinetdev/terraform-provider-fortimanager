...
func resourceObjectSwitchControllerQosIpDscpMapCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSwitchControllerQosIpDscpMap(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSwitchControllerQosIpDscpMap(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSwitchControllerQosIpDscpMapUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSwitchControllerQosIpDscpMap(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSwitchControllerQosIpDscpMap resource: %v", err)
	}
...
func resourceObjectSwitchControllerQosIpDscpMapDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSwitchControllerQosIpDscpMap(mkey, paradict, wsParams)
	if err != nil {
