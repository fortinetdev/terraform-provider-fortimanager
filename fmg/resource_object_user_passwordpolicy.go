...
func resourceObjectUserPasswordPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectUserPasswordPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectUserPasswordPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectUserPasswordPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectUserPasswordPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectUserPasswordPolicy resource: %v", err)
	}
...
func resourceObjectUserPasswordPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectUserPasswordPolicy(mkey, paradict, wsParams)
	if err != nil {
