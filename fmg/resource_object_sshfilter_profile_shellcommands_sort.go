...
func resourceObjectSshFilterProfileShellCommandsSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectSshFilterProfileShellCommandsSortCreateUpdate,
		Read:   resourceObjectSshFilterProfileShellCommandsSortRead,
		Update: resourceObjectSshFilterProfileShellCommandsSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectSshFilterProfileShellCommandsSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectSshFilterProfileShellCommandsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectSshFilterProfileShellCommands: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectSshFilterProfileShellCommandsSortRead(d, m)
}

func resourceObjectSshFilterProfileShellCommandsSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectSshFilterProfileShellCommandsSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectSshFilterProfileShellCommands sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectSshFilterProfileShellCommands List for (%s): %s", d.Id(), err)
		}
	} else {
