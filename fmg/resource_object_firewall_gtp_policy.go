...
func resourceObjectFirewallGtpPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallGtpPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallGtpPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallGtpPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallGtpPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallGtpPolicy resource: %v", err)
	}
...
func resourceObjectFirewallGtpPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallGtpPolicy(mkey, paradict, wsParams)
	if err != nil {
