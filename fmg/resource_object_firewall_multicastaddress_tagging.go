...
func resourceObjectFirewallMulticastAddressTaggingCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallMulticastAddressTagging(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallMulticastAddressTagging(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallMulticastAddressTaggingUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallMulticastAddressTagging(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallMulticastAddressTagging resource: %v", err)
	}
...
func resourceObjectFirewallMulticastAddressTaggingDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallMulticastAddressTagging(mkey, paradict, wsParams)
	if err != nil {
