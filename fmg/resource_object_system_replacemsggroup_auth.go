...
func resourceObjectSystemReplacemsgGroupAuthCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectSystemReplacemsgGroupAuth(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectSystemReplacemsgGroupAuth(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectSystemReplacemsgGroupAuthUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectSystemReplacemsgGroupAuth(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectSystemReplacemsgGroupAuth resource: %v", err)
	}
...
func resourceObjectSystemReplacemsgGroupAuthDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectSystemReplacemsgGroupAuth(mkey, paradict, wsParams)
	if err != nil {
