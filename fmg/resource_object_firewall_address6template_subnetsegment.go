...
func resourceObjectFirewallAddress6TemplateSubnetSegmentCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallAddress6TemplateSubnetSegment(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallAddress6TemplateSubnetSegment(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallAddress6TemplateSubnetSegmentUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallAddress6TemplateSubnetSegment(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallAddress6TemplateSubnetSegment resource: %v", err)
	}
...
func resourceObjectFirewallAddress6TemplateSubnetSegmentDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallAddress6TemplateSubnetSegment(mkey, paradict, wsParams)
	if err != nil {
