...
func resourceObjectIsolatorProfileEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectIsolatorProfileEntriesSortCreateUpdate,
		Read:   resourceObjectIsolatorProfileEntriesSortRead,
		Update: resourceObjectIsolatorProfileEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectIsolatorProfileEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectIsolatorProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectIsolatorProfileEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectIsolatorProfileEntriesSortRead(d, m)
}

func resourceObjectIsolatorProfileEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectIsolatorProfileEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectIsolatorProfileEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectIsolatorProfileEntries List for (%s): %s", d.Id(), err)
		}
	} else {
