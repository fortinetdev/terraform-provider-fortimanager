...
func resourcePackagesFirewallShapingPolicySort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallShapingPolicySortCreateUpdate,
		Read:   resourcePackagesFirewallShapingPolicySortRead,
		Update: resourcePackagesFirewallShapingPolicySortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallShapingPolicySortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallShapingPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallShapingPolicy: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallShapingPolicySortRead(d, m)
}

func resourcePackagesFirewallShapingPolicySortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallShapingPolicySort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallShapingPolicy sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallShapingPolicy List for (%s): %s", d.Id(), err)
		}
	} else {
