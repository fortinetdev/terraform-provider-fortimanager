...
func resourceObjectDynamicVipCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectDynamicVip(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectDynamicVip(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectDynamicVipUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectDynamicVip(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectDynamicVip resource: %v", err)
	}
...
func resourceObjectDynamicVipDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectDynamicVip(mkey, paradict, wsParams)
	if err != nil {
