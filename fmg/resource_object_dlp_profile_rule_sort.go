...
func resourceObjectDlpProfileRuleSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectDlpProfileRuleSortCreateUpdate,
		Read:   resourceObjectDlpProfileRuleSortRead,
		Update: resourceObjectDlpProfileRuleSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectDlpProfileRuleSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectDlpProfileRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectDlpProfileRule: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectDlpProfileRuleSortRead(d, m)
}

func resourceObjectDlpProfileRuleSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectDlpProfileRuleSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectDlpProfileRule sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectDlpProfileRule List for (%s): %s", d.Id(), err)
		}
	} else {
