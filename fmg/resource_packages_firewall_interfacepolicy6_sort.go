...
func resourcePackagesFirewallInterfacePolicy6Sort() *schema.Resource {
	return &schema.Resource{
		Create: resourcePackagesFirewallInterfacePolicy6SortCreateUpdate,
		Read:   resourcePackagesFirewallInterfacePolicy6SortRead,
		Update: resourcePackagesFirewallInterfacePolicy6SortCreateUpdate,
		Delete: schema.Noop,
...
func resourcePackagesFirewallInterfacePolicy6SortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdatePackagesFirewallInterfacePolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting PackagesFirewallInterfacePolicy6: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourcePackagesFirewallInterfacePolicy6SortRead(d, m)
}

func resourcePackagesFirewallInterfacePolicy6SortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadPackagesFirewallInterfacePolicy6Sort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading PackagesFirewallInterfacePolicy6 sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading PackagesFirewallInterfacePolicy6 List for (%s): %s", d.Id(), err)
		}
	} else {
