...
func resourceObjectFirewallGtpIpPolicyCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallGtpIpPolicy(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallGtpIpPolicy(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallGtpIpPolicyUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallGtpIpPolicy(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallGtpIpPolicy resource: %v", err)
	}
...
func resourceObjectFirewallGtpIpPolicyDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallGtpIpPolicy(mkey, paradict, wsParams)
	if err != nil {
