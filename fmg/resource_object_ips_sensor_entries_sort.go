...
func resourceObjectIpsSensorEntriesSort() *schema.Resource {
	return &schema.Resource{
		Create: resourceObjectIpsSensorEntriesSortCreateUpdate,
		Read:   resourceObjectIpsSensorEntriesSortRead,
		Update: resourceObjectIpsSensorEntriesSortCreateUpdate,
		Delete: schema.Noop,
...
func resourceObjectIpsSensorEntriesSortCreateUpdate(d *schema.ResourceData, m interface{}) (err error) {
	...
	err = c.CreateUpdateObjectIpsSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error sorting ObjectIpsSensorEntries: %s", err)
	}

	d.SetId(sortby + sortdirection)

	return resourceObjectIpsSensorEntriesSortRead(d, m)
}

func resourceObjectIpsSensorEntriesSortRead(d *schema.ResourceData, m interface{}) error {
	mkey := d.Id()
	...
	sorted, o, err := c.ReadObjectIpsSensorEntriesSort(&input_model)
	if err != nil {
		return fmt.Errorf("Error reading ObjectIpsSensorEntries sort status: %s %s", err, mkey)
	}
	...
	if o != nil {
		if err := d.Set("state_list", o); err != nil {
			log.Printf("[WARN] Error reading ObjectIpsSensorEntries List for (%s): %s", d.Id(), err)
		}
	} else {
