...
// sortPackagesFirewallAclItem contains the parameters for each item
type sortPackagesFirewallAclItem struct {
	policyid int
}

func getEntryListPackagesFirewallAcl(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortPackagesFirewallAclItem, err error) {
	...
	if err == nil {
		...
		var members []sortPackagesFirewallAclItem
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortPackagesFirewallAclItem{
...
func bEntryListSortedPackagesFirewallAcl(itemList []sortPackagesFirewallAclItem, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterPackagesFirewallAcl(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListPackagesFirewallAcl(itemList []sortPackagesFirewallAclItem, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortPackagesFirewallAclItem
	if sortby == "policyid" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortPackagesFirewallAclItem)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterPackagesFirewallAcl(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdatePackagesFirewallAclSort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdatePackagesFirewallAclSort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListPackagesFirewallAcl(c, inputModel)
	...
	bsorted := bEntryListSortedPackagesFirewallAcl(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListPackagesFirewallAcl(itemList, c, inputModel)
	if err != nil {
...
// ReadPackagesFirewallAclSort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadPackagesFirewallAclSort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListPackagesFirewallAcl(c, inputModel)
	if err != nil {
