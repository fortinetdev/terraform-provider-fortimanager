...
// sortPackagesFirewallAcl6Item contains the parameters for each item
type sortPackagesFirewallAcl6Item struct {
	policyid int
}

func getEntryListPackagesFirewallAcl6(c *FortiSDKClient, inputModel *SortInputModel) (itemList []sortPackagesFirewallAcl6Item, err error) {
	...
	if err == nil {
		...
		var members []sortPackagesFirewallAcl6Item
		for _, v := range listTmp {
			c := v.(map[string]interface{})

			members = append(members,
				sortPackagesFirewallAcl6Item{
...
func bEntryListSortedPackagesFirewallAcl6(itemList []sortPackagesFirewallAcl6Item, inputModel *SortInputModel) (bsorted bool) {
...
func moveAfterPackagesFirewallAcl6(idbefore, idafter int, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
...
func sortEntryListPackagesFirewallAcl6(itemList []sortPackagesFirewallAcl6Item, c *FortiSDKClient, inputModel *SortInputModel) (err error) {
	sortby := inputModel.SortBy
	sortdirection := inputModel.SortDirection
	manual_order := inputModel.ManualOrder
	var targetItemOrder []sortPackagesFirewallAcl6Item
	if sortby == "policyid" {
		if sortdirection == "ascending" {
			...
			curItemMap := make(map[string]sortPackagesFirewallAcl6Item)
	...
	for i := 0; i < len(targetItemOrder)-1; i++ {
		err = moveAfterPackagesFirewallAcl6(targetItemOrder[i+1].policyid, targetItemOrder[i].policyid, c, inputModel)
		if err != nil {
...
// CreateUpdatePackagesFirewallAcl6Sort API operation for FortiManager to sort the firewall policies.
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) CreateUpdatePackagesFirewallAcl6Sort(inputModel *SortInputModel) (err error) {
	itemList, err := getEntryListPackagesFirewallAcl6(c, inputModel)
	...
	bsorted := bEntryListSortedPackagesFirewallAcl6(itemList, inputModel)
	if bsorted == true {
		return
	}

	err = sortEntryListPackagesFirewallAcl6(itemList, c, inputModel)
	if err != nil {
...
// ReadPackagesFirewallAcl6Sort API operation for FortiManager to read the firewall policies sort results
// Returns sort status
// Returns error for service API and SDK errors.
func (c *FortiSDKClient) ReadPackagesFirewallAcl6Sort(inputModel *SortInputModel) (sorted bool, itemMapList []interface{}, err error) {
	itemList, err := getEntryListPackagesFirewallAcl6(c, inputModel)
	if err != nil {
