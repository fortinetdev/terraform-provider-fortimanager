...
~> This resource is a sub resource for variable `max_url_param` of resource `fortimanager_object_waf_profile_constraint`. Conflict and overwrite may occur if use both of them.
...
```hcl
resource "fortimanager_object_waf_profile_constraint_maxurlparam" "trname" {
  action     = "block"
  log        = "enable"
