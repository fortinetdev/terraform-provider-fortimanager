...
~> This resource is a sub resource for variable `malformed` of resource `fortimanager_object_waf_profile_constraint`. Conflict and overwrite may occur if use both of them.
...
```hcl
resource "fortimanager_object_waf_profile_constraint_malformed" "trname" {
  action     = "block"
  log        = "enable"
