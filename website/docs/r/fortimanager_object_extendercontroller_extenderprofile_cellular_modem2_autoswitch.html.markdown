...
~> This resource is a sub resource for variable `auto_switch` of resource `fortimanager_object_extendercontroller_extenderprofile_cellular_modem2`. Conflict and overwrite may occur if use both of them.
...
```hcl
resource "fortimanager_object_extendercontroller_extenderprofile_cellular_modem2_autoswitch" "trname" {
  dataplan             = "enable"
  disconnect           = "enable"
